...
import api, {getUseractives, getAtividades} from '../../services/api';
...
function Criar({navigation}) {
  ...
  useEffect(() => {
    ...
    async function getStorage() {
      const awaitt = await getAtividades();
      setAtividades(awaitt.data);
    }
    ...
    <Modal animationType="slide" visible={modalVisible}>
      <View style={estiloTelaCriacao.modal}>
        <TouchableOpacity
          ...
          style={estiloTelaCriacao.botaoVoltar}>
        ...
        <View style={estiloTelaCriacao.caixa1}>
          ...
          <View style={estiloTelaCriacao.caixaDate}>
            <Calendar
              style={estiloTelaCriacao.icones}
              name="calendar"
              size={10}
              color={'gray'}>
              {' '}
          ...
          <View style={estiloTelaCriacao.emoji}>
            {Humorr.map(({nome, source, id, name}) => {
              ...
              return (
                <TouchableOpacity
                  key={id}
                  onPress={() => pegarhumor(name, id)}
                  style={estiloTelaCriacao.humores}>
                  <Image
                    style={[
                      estiloTelaCriacao.img,
                      {
        ...
        <TouchableOpacity
          style={estiloTelaCriacao.botaao}
          onPress={() => {
            [postApi(user), navigation.navigate('MyTabs')];
          }}>
